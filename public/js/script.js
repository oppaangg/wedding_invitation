...
document.addEventListener('mouseup', () => { userInteracting = false; });
...
function submitRsvp() {
    ...
    const isConfirmed = document.querySelector('input[name=is_confirmed]:checked')?.value;
    ...
    fetch(window.RSVP_ROUTE, {
        ...
        body: JSON.stringify({ is_confirmed: isConfirmed, guest_count: guestCount, message })
    })
    .then(r => r.json())
    .then(() => {
